...
For each directory in `golden/.claude/skills/*/`, check whether a matching directory exists in the project's `.claude/skills/`. Compare the `SKILL.md` files within each:
...
**New skills:** Copy the entire skill directory from golden to the project's `.claude/skills/`. Create `.claude/skills/` if it doesn't exist.

**Updated skills:** Replace the project's `SKILL.md` with the golden version.
...
- Skills added: /skill-name
- Skills updated: /skill-name
...
- Project-specific skills: preserved
...
- NEVER remove project-specific skills (those not in the golden set)
