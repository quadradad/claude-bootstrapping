...
**Skills:** For each skill directory in the target project's `.claude/skills/`, compare `SKILL.md` against the golden set equivalent. Classify as:
- **Golden original** — file exists in both, content matches → skip
- **Golden modified** — file exists in both, content differs → candidate for extraction
- **Novel** — skill exists in target but not in `golden/.claude/skills/` → candidate for new golden skill
- **Project-specific** — skill was generated by `/bootstrap-claude` for this project → skip, not generalizable
...
9. **Modified skills** — show the diff, propose whether the change is universal (update golden) or project-specific (skip)
10. **Novel skills** — show the skill content, propose a generalized version that strips project-specific references. Recommend: include in golden set, or skip.
...
- For novel skills, create the new directory in `golden/.claude/skills/<name>/` with `SKILL.md`
- For modified skills, update `golden/.claude/skills/<name>/SKILL.md`
...
- golden/.claude/skills/[name]/SKILL.md: [created / updated]
